...
class Car:
    ...
    def draw_collision(self, screen):
        pos = (int(self.center[0]), int(self.center[1]))
        if self.collision:
            self.time_from_collision = 10
            pygame.draw.circle(screen, (255, 255, 0), pos, 20)
        else:
            self.time_from_collision = max(0., self.time_from_collision - 0.5)
        pygame.draw.circle(screen, (255, 0, 0), pos, self.time_from_collision)
...
def run_simulation(agents, map="white", render=False):
    ...
    positions = [[] for _ in range(len(agents))]
    ss = time.time()
    ...
    for j in range(250):
        ...
        for i in range(len(agents)):
            ...
            outtime = time.time()
            ob = cars[i].get_data()
            positions[i].append([cars[i].center[0], cars[i].center[1]])
